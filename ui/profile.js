  ...
  body.innerHTML = `
    ...
    <div class="profile-title" id="profile-title">${state.currentTitle || 'Новичок'}</div>
      ...
      <div class="profile-level-bar-container">
        <img src="rank/${rankData.icon}" class="profile-rank-icon" alt="${rankData.name}" loading="lazy" />
...
export function renderQuests(state, dependencies) {
  ...
  const weeklyHtml = state.quests.weekly
    .map((challenge) => {
  ...
  const dailyQuestsHtml = state.quests.daily
    .map((quest) => {
