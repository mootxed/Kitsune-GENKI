...
export function renderSensei(state, dependencies) {
  ...
  if (chatHistory.length === 0) {
    addBotMessage("こんにちは！Я — Kitsune Sensei 🦊 Спросите что угодно про японский язык или учебник Genki!", state, dependencies);
  } else {
  ...
  $("#chat-input").addEventListener("keydown", (e) => { if (e.key === "Enter") sendChat(state, dependencies); });
