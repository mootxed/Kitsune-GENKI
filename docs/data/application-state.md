...
В этом документе приведено детальное описание централизованного состояния веб-приложения **KotoKitsu** (`state/store.js`).

---

## 📌 Версионирование состояния

- **Текущая версия схемы**: `STATE_SCHEMA_VERSION = 17` (см. [справочник версий](../reference/generated-versions.md)).
- **Ключ хранения в localStorage (Legacy/Fallback)**: `kitsune_state_v1` (сохранён ради обратной совместимости).
- **Ключ хранения в IndexedDB (Основное)**: `app_state` store → запись с ID `'state'`.

---

## 🧱 Структура объектов State (Упрощённый пример)

```javascript
// Упрощенный пример структуры приложения
{
  version: 17,                     // Идентификатор версии схемы (STATE_SCHEMA_VERSION)
